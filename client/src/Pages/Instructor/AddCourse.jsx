...
const AddCourse = () => {
  ...
  const handleLesson = (action, moduleId, lessonIndex) => {
    ...
    console.log("Before updating module:", lessonDetails); // 🔍 Debugging
    ...
    setModules(prevModules => {
      return prevModules.map(module => {
        if (module.moduleId === currentModuleId) {
          ...
          console.log("New lesson being added:", newLesson); // 🔍 Log new lesson
    ...
    // ✅ Preserve `lessonFile` until after lesson is added
    setTimeout(() => {
      console.log("Resetting lessonDetails..."); // 🔍 Debug reset
      setLessonDetails(prev => ({
        ...prev,
        lessonTitle: '',
        lessonDuration: '',
        lessonQuiz: '',
        // 🚀 Do NOT reset `lessonFile` immediately
      }));
    }, 100);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (lessonDetails.lessonFile) {
        console.log("Appending file:", lessonDetails.lessonFile); // Debugging
      ...
      for (let pair of formData.entries()) {
        console.log(pair[0], pair[1]); // Check if lesson files exist
      }

      console.log("Lesson Details Before Submitting:", lessonDetails);


      const { data } = await axios.post(`${backend_url}/instructor/courses`, formData, {
        headers: {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start 
      ...
      <form onSubmit={handleSubmit} encType='multipart/form-data' className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor='thumbnailImage' className='flex items-center gap-3'>
              ...
              <img className='max-h-10 w-18' src={image ? URL.createObjectURL(image) : ''} alt="" />
            </label>
          </div>
        </div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.files.length > 0) {
                          ...
                          console.log("File selected before update:", selectedFile); // ✅ Log before updating

                          setLessonDetails((prev) => {
                            const updatedDetails = { ...prev, lessonFile: selectedFile };
                            console.log("Updated lessonDetails inside state update:", updatedDetails); // ✅ Log before state change
